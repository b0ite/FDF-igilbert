...
/*   Updated: 2025/02/18 11:08:40 by igilbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	fdf_calc_offset_top(t_fdf *fdf)
{
	t_offset	of;
	t_point		point;
...
{
	if (fdf->view == 0)
		fdf_calc_offset_iso(fdf);
	else if (fdf->view == 1)
		fdf_calc_offset_parallel(fdf);
	else if (fdf->view == 2)
		fdf_calc_offset_top(fdf);
}
...
t_point	fdf_calculate_iso(t_fdf *fdf, int i, int j)
{
	...
	point.draw_y = -point.origin_z + (point.origin_x + point.origin_y)
		* sin(0.523599);
